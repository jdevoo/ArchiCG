    ...
    onClick: function(event) {
        ...
        if (event.target=="neighborhood"){            
            ...
            if (depths.includes(theDepth)){
                ...
                let startingSelection=cy.nodes(':selected');
...
`)};
      ...
      if (event.target=="shortpath"){
        ...
        let dijkstra = cy.elements(myFilter).dijkstra('#'+document.getElementById('pathsource').value,
        function(){return 1
          //this.data('weight')
          ;},ACGTraversalDirected);
        let pathTo = dijkstra.pathTo( cy.$('#'+document.getElementById('pathtarget').value) );
        let distToJ = dijkstra.distanceTo( cy.$('#'+document.getElementById('pathtarget').value) );
        let thePath=[];
