...
// All field state lives in the DOM inputs (read/written by id) — no
// module-level degree variables are needed.
...
Computes graph-theory degrees.

"Calculate from selected node" reports the selected node's Degree, InDegree
and OutDegree.

"Calculate from: Selection / Visible / Full" reports the aggregate Total, Min
and Max degrees (overall, incoming and outgoing) over the chosen set of nodes.

For each measure, the plain field excludes self-loops and the "IL" field
includes them. Degrees are computed on the flat graph, not on compound nesting.
</pre>`)};
        if (event.target=="calculatefromnode"){
         var selection = cy.nodes(':selected');
         if (selection.length !=1) {w2alert("A single node must be selected!")}
         else{
            //alert ("a single node is selected having the id: "+ selection[0].id())
            let myNode=selection[0];
        ...
        if (event.target=="calculatefromselection"){
            var selection = cy.nodes(':selected');
            if (selection.length === 0){ w2alert('Select one or more nodes first.'); return; }
...
function calculateDegreesForGraph(selection){
    ...
    document.getElementById('theMaxOutDegree').value=selection.maxOutdegree(false);
    ...
    document.getElementById('theMaxOutDegreeIL').value=selection.maxOutdegree(true);
