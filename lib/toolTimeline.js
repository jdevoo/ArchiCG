...
var toolbarTimelineDefinition={
    ...
    onClick: function(event) {
      if (event.target=="timeline2"){
        let filterPlannedWork=`[${document.getElementById("globalPlanStartDataProperty").value}][${document.getElementById("globalPlanEndDataProperty").value}]`
        let plannedWork=cy.nodes().union(api.getAllCollapsedChildrenRecursively(node)).filter(filterPlannedWork);
        ...
        let filterActualFinishedWork=`[${document.getElementById("globalActualStartDataProperty")}][${document.getElementById("globalActualEndDataProperty")}]`
        let finishedWork=cy.nodes(`${filterActualFinishedWork}`)
...
function tagSelectionWithColor(e){
  ...
  function searchNode(nodeId) {
    ...
    let targetnode= cy.nodes().filter(node => node.data('id') === nodeId);
