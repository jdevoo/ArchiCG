...
function oneLetterRelationCode(relation){
  let letterIndex=acg_ArchiMateRelations.indexOf(relation.data("type"));
  if (letterIndex+1){return ArchiMateRelationIDs[letterIndex]} else {return undefined}
 }

function allowedArchiMateRelation(edge){
  let letter=oneLetterRelationCode(edge);
  let sourceType=edge.source().data("type");
  let targetType=edge.target().data("type");
  let sourceIndex=ja_ArchiMateObjects.indexOf(sourceType);
  let targetIndex=ja_ArchiMateObjects.indexOf(targetType);
