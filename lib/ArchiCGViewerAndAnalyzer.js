...
// Clone, so the active layout is a separate object — staging fCoSE inputs into
// cyLayout must not mutate defaultCyLayout, or "Restore Default" can't restore.
var cyLayout = Object.assign({}, defaultCyLayout);
var acgLayout = "fcose";
var cy;
var ur;
var undoRedo = true;
...
function toolDisplayName(tool) {
  // Friendly label for a tool id — shared by the Tools menu and the left-panel
  // title so they stay in sync. Falls back to the raw id if unmapped.
  var names = {
    archicgEditor: 'Editor',
    ACGTraversal: 'Traversal',
    ACGDegrees: 'Degrees',
    PropertyColoredMap: 'Colored Map',
    Coloring: 'Colouring',
    GraphAsMatrix: 'Graph as Matrix',
    Fcose: 'Layout (fCoSE)',
    URLNavigation: 'URL Navigation',
    Timeline: 'Timeline',
    Car: 'Car (Tutorial)'
  };
...
function activateToolBar(tool) {
  w2ui.layoutToolbars.set("main", { title: toolDisplayName(tool) });
...
var config = {
  ...
  layoutToolbars: {
    ...
    panels: [
      {
        title: toolDisplayName(selectedTool), type: 'main',
        size: 200,
        resizable: true,
        minSize: 35
      }
  ...
  gridProperties: {
    ...
    toolbar: {
      items: [
        { id: 'add', type: 'button', text: 'Add', icon: 'w2ui-icon-plus' },
        { id: 'remove', type: 'button', text: 'Remove', icon: 'w2ui-icon-cross' },
      ],
      onClick: function (event) {
        let el = cy.$id(w2ui.form2.getValue("identifier"));
        if (event.target == 'add') {
          w2ui.gridProperties.add({ recid: w2ui.gridProperties.records.length + 1 });
        }
        if (event.target == 'remove') {
          let sel = w2ui.gridProperties.getSelection();
          if (!sel || sel.length === 0) { w2alert('Select a property row to remove.'); return; }
          sel.forEach(function (s) {
            let recid = (s && s.recid !== undefined) ? s.recid : s;
            let rec = w2ui.gridProperties.get(recid);
            if (rec && rec.fname && el.length) { el.removeData(rec.fname); }
            w2ui.gridProperties.remove(recid);
          });
    ...
    // No Save button: each inline edit (type → Enter) commits straight to the
    // selected element's data. In-memory only — File → Save persists to disk.
    // NB: renaming a property name adds the new key; use Remove to drop the old one.
    onChange: function (event) {
      event.onComplete = function () {
        let el = cy.$id(w2ui.form2.getValue("identifier"));
        if (el.length === 0) return;
        // Effective cell value = pending inline edit if present, else committed.
        let eff = function (rec, f) {
          return (rec.w2ui && rec.w2ui.changes && rec.w2ui.changes[f] !== undefined)
            ? rec.w2ui.changes[f] : rec[f];
        };
        w2ui.gridProperties.records.forEach(function (rec) {
          let name = eff(rec, 'fname');
          if (name) { el.data(name, eff(rec, 'lvalue')); }
        });
        // We've committed the edit to the graph, so fold it into the records and
        // clear the red "changed cell" markers — there's no pending save state.
        if (typeof w2ui.gridProperties.mergeChanges === 'function') { w2ui.gridProperties.mergeChanges(); }
        cy.style().update();
      };
    },
  },
  ...
  gridAttributes: {
    ...
    columns: [
      { field: 'recid', text: 'ID', size: '50px', sortable: true, resizable: true },
      { field: 'key', text: 'Key', size: '50px', sortable: true, resizable: true },
      { field: 'order', text: 'Order', size: '50px', sortable: true, resizable: true },
      { field: 'AttributeId', text: 'Id', size: '50px', sortable: true, resizable: true },
      { field: 'label', text: 'Label', size: '30%', sortable: true, searchable: true },
      { field: 'type', text: 'Type', size: '30%', sortable: true, searchable: true },
      { field: 'source', text: 'Source', size: '30%', sortable: true, searchable: true },
      { field: 'description', text: 'Description', size: '70%', sortable: true, searchable: true }
    ],
    // Read-only view — Entity Attributes are a nested structure populated on
    // double-click; no add/edit/save controls.
  },
  ...
  gridEdges: {
    ...
    // Read-only view — lists the real edges hidden inside a collapsed edge
    // bundle; nothing to add, edit, or save here.
  },
  ...
  form: {
    ...
    // No buttons: label/description commit to the graph on change (blur/Enter),
    // consistent with the property grids. Everything is in-memory — reload the
    // file (File → Load) to discard unsaved changes.
    onChange: function (event) {
      event.onComplete = function () {
        let el = cy.$id(w2ui.form2.getValue("identifier"));
        if (el.length === 0) return;
        el.data("label", w2ui.form.getValue("label"));
        el.data("description", w2ui.form.getValue("description"));
        // The node label is drawn by a function style mapper (calculateLabel),
        // which doesn't re-run on a bare data() change — force a restyle so the
        // edited label repaints on the Landscape immediately.
        cy.style().update();
      };
    }
  },
  ...
  mainmenu: {
    ...
    items: [
      {
        ...
        items: [
          ...
          },
          {
            ...
            items: [
              { id: 'id8', text: 'CSV' },
              { id: 'owlexport', text: 'JSON-LD' },
              { id: 'pngexport', text: 'PNG' },
              { id: 'svgexport', text: 'SVG' },
            ]
          }
        ...
        items: [
          ...
          },
          {
            ...
            items: [
              ...
              { id: 'id9', text: 'Grabify non selected' }
            ]
          },
          {
            ...
            items: [
              ...
              { id: 'id14', text: 'Unlock non selected' }
            ]
          },
          {
    ...
    onClick: function (event) {
      ...
      if (event.target == "submenu2:pngexport") {
        var matrixView = (typeof isMatrixViewVisible === "function") && isMatrixViewVisible();
        var fileNameToSave = matrixView ? "ArchiCG-matrix.png" : "ArchiCG.png";
        w2prompt({ label: 'Enter value', value: fileNameToSave, attrs: 'size=6' })
          .change((event) => { fileNameToSave = event.detail.originalEvent.target.value })
          .ok(() => {
            try {
              if (matrixView) { exportMatrixAsPNG(fileNameToSave); }
              else { saveAs(cy.png({ bg: "white", full: "true" }), fileNameToSave); }
            } catch (error) { }
          });

      }
      if (event.target == "submenu2:svgexport") {
        var matrixView = (typeof isMatrixViewVisible === "function") && isMatrixViewVisible();
        var fileNameToSave = matrixView ? "ArchiCG-matrix.svg" : "ArchiCG.svg";
        w2prompt({
          label: 'Enter value',
          ...
          .ok(() => {
            try {
              if (matrixView) {
                exportMatrixAsSVG(fileNameToSave);
              } else {
      ...
      if (event.target == "submenu3:id3") {
        if (cy.nodes(":selected").length < 1) { w2alert('Select one or more nodes to collapse recursively.'); return; }
      ...
      if (event.target == "submenu3:id5") {
        if (cy.nodes(":selected").length < 1) { w2alert('Select one or more nodes to expand recursively.'); return; }
      ...
      if (event.target == "submenu3:id6") {
        ...
        if (selection.length < 1) { w2alert('Select one or more nodes to group into a new compound.'); return; }
      ...
      if (event.target == "submenu3:id7") {
        ...
        if (elems.length < 1) { w2alert('Select a compound node (one that contains other nodes) to remove.'); return; }
      ...
      if (event.target == "submenu3:id8") {
        ...
        if (selection.length == 1) {
          ...
          w2alert('Select exactly one node to nest a new node inside it.');
        }
      }
      ...
      if (event.target == "submenu3:id11") {
        if (cy.edges(".cy-expand-collapse-collapsed-edge").length > 0) {
          ...
          w2alert('There are no collapsed edges to expand.');
        }
      }
      ...
      if (event.target == "submenu3:id12") {
        ...
        if (edges.length >= 2) {
          ...
          w2alert('Select at least two edges to collapse them into one.');
        }
      }
      ...
      if (event.target == "submenu3:id13") {
        ...
        if (edges.length > 0) {
          ...
          w2alert('Select one or more collapsed edges to expand.');
        }
      }

      if (event.target == "submenu3:id14") {
        if (cy.nodes(":selected").length < 2) { w2alert('Select two or more nodes to collapse the edges between them.'); return; }
      ...
      if (event.target == "submenu3:id15") {
        if (cy.nodes(":selected").length < 2) { w2alert('Select two or more nodes to expand the edges between them.'); return; }
      ...
      if (event.target == "submenu4:id1") {
        ...
        if (selectedNodes.length < 1) { w2alert('Select one or more nested nodes to pull out into the graph.'); return; }
        let toUnnest = [];
        ...
        if (toUnnest.length === 0 && !touchedEdge) {
          w2alert('None of the selected nodes take part in a composition relationship.');
          return;
        }
      ...
      if (event.target == "submenu4:id2") {
        ...
        var selectedNodes = cy.nodes(":selected");
        if (selectedNodes.length < 1) { w2alert('Select one or more nodes to nest by their composition relationships.'); return; }
        let relationIDs = [];
        cy.batch(function () {
          selectedNodes.forEach(function (node) {
            node.outgoers().forEach(function (rel) {
        ...
        if (relationIDs.length === 0) {
          w2alert('None of the selected nodes have outgoing composition relationships to nest.');
          return;
        }
      ...
      if (event.target == "submenu4:id4") {
        ...
        // Reflect the active state as a radio marker on the Show/Hide edge pair.
        this.get('submenu4').items.forEach(function (it) {
          if (it.id === 'id4') { it.text = '● Show edge'; }
          if (it.id === 'id5') { it.text = '○ Hide edge'; }
        });
        this.refresh();
      }
  ...
  toolbararchicgEditorDefinition: {
    ...
    items: [
      ...
      {
        type: 'html', id: 'input00A', html: `<div style=" height: 22px;display: flex;
          align-items: center;">Bulk-create numbered nodes</div>`},
      { type: 'new-line' },
      {
        type: 'html', id: 'input00B', html: `<div style=" height: 20px;display: flex;
          align-items: center;">Start <input id="iterationStart" title="First number" style="color:blue;" value="1" size="2"> Step <input id="iterationStep" title="Increment between numbers" style="color:blue;" value="1" size="2"> Digits <input id="iteratorLength" title="Zero-padding width (3 → 001)" style="color:blue;" value="3" size="3"></div>` },
      { type: 'new-line' },
      {
        type: 'html', id: 'input00C', html: `<div style=" height: 20px;display: flex;
          align-items: center;">Count <input id="numberOfIterations" title="How many nodes to create" style="color:blue;" value="1" size="2" ></div>` },
      { type: 'spacer' },
      { type: 'button', id: 'createNodes', text: 'Create', tooltip: 'Create Count nodes of the current Node type, labelled <Node type>-NNN, numbered from Start by Step and zero-padded to Digits' }
    ],
    onClick: function (event) {
...
function toggleBottomCollapse() {
  ...
  // Re-sync cytoscape to the new graph-area height so the graph and the minimap
  // (which follows cy's 'resize' event) update to match.
...
function main() {
  ...
  $('#archicg-layout').w2layout({
    ...
    panels: [
      ...
      // border-top:0 — this panel has no native title; its "Editor" title comes
      // from the nested layoutToolbars layout, which otherwise sits 1px lower than
      // the native titles of the main/right panels due to this panel's top border.
      { type: 'left', size: "22%", resizable: true, style: pstyle + ';border-top:0px', html: 'left' },
      {
        type: 'main', title: "Landscape", header: "Landscape", style: pstyle, resizable: true, overflow: 'hidden', html: `
    ...
    // Cytoscape doesn't observe container resizes — when a panel divider is
    // dragged (or the bottom panel is collapsed/expanded), tell cy its container
    // changed so the graph and the bird's-eye minimap (which follows cy's
    // 'resize' event) re-render to the new size.
  ...
  tools.forEach(function (tool) {
    ...
    toolsMenuItems.push({ type: 'button', id: tool, text: toolDisplayName(tool) });
  })
...
Access it using the  Tools menu.`)
    ...
    w2ui.sidebar.select('mainproperties');
    // select() only highlights the sidebar row — it does NOT fire the sidebar
    // onClick that swaps the main panel content. Without this, double-clicking a
    // node highlights "Label and Description" but leaves the previous grid (e.g.
    // EA Properties) showing. Mirror the 'mainproperties' case so the panel matches.
    w2ui.layoutProperties.html('main', w2ui.form);
    $(w2ui.layoutProperties.el('main')).removeClass('w2ui-grid').css({ 'border-left': '1px solid silver' });
  }
